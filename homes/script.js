...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updatePieChart() {
    const totalIncome = lineChart.data.datasets[0].data.reduce(
      (sum, value) => sum + value,
      0
    );
    const totalExpenses = lineChart.data.datasets[1].data.reduce(
      (sum, value) => sum + value,
      0
    );
...
// Function to update progress bar based on income and expenses
// script.js
function updateProgressBar() {
  const totalBudget = parseFloat(document.getElementById("total-budget").value);
  const amountSpent = parseFloat(document.getElementById("amount-spent").value);

  if (isNaN(totalBudget) || isNaN(amountSpent) || totalBudget <= 0) {
    alert("Please enter valid numbers for both Total Budget and Amount Spent.");
    return;
  }
  ...
  if (percentageSpent >= 100) {
    ...
    progressBar.style.backgroundColor = "#50abd5";
  }
}
